...
class DistalEvaluation(CleanEvaluation):
    ...
    def __init__(self, clean_probabilities, distal_probabilities, labels, validation=0.1, errors=None, include_misclassifications=False, detector=common.numpy.max_detector, clean_scores=None, distal_scores=None):
        """
        Constructor.
        ...
        :param distal_scores: scores corresponding to clean_probabilities
        :type distal_scores: numpy.ndarray
        """
        ...
        assert not (detector is not None and (clean_scores is not None or distal_scores is not None))
        assert (distal_scores is not None and clean_scores is not None) or (distal_scores is None and clean_scores is None)
        ...
        if errors is not None:
            ...
            if distal_scores is not None:
                distal_scores = distal_scores[selected, numpy.arange(distal_scores.shape[1])]
            if clean_scores is not None:
                reference_scores = clean_scores[:self.N]
            self.A = 1  # !
        else:
            ...
            if distal_scores is not None:
                distal_scores = distal_scores.reshape(self.A * self.N,)
            if clean_scores is not None:
                reference_scores = numpy.tile(clean_scores[:self.N], (self.A))
        ...
        if distal_scores is not None and clean_scores is not None:
            assert distal_scores.shape[0] == reference_scores.shape[0]
            assert len(distal_scores.shape) == len(reference_scores.shape)
        ...
        #self.test_confidences = self.test_probabilities[numpy.arange(self.test_AN), self.test_predictions]
        ...
        if clean_scores is not None:
            self.test_confidences = reference_scores
        ...
        if validation > 0:
            ...
            if clean_scores is not None:
                self.validation_confidences = clean_scores[self.validation_N:]
        ...
        self.test_distal_confidences = None
        """ (numpy.ndarray) Test confidences. """

        if detector is not None:
            self.test_distal_confidences = detector(self.test_distal_probabilities)
        if clean_scores is not None:
            self.test_distal_confidences = distal_scores
        assert self.test_distal_confidences.shape[0] == self.test_distal_probabilities.shape[0]
